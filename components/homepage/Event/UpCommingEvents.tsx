...
function UpcomingEvents({ hideHeader = false }: { hideHeader?: boolean }) {
  ...
  useEffect(() => {
    const fetchUpcoming = async () => {
      try {
        ...
        const upcoming = (data || []).filter(e => e.status === 'Upcoming').slice(0, 5);
        setEvents(upcoming);
      } catch (error) {
        ...
        <div className="flex items-center justify-between gap-2.5 mb-8">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-2xl font-bold text-gray-900">
              Upcoming <span className="text-brand font-light">Events</span>
            </h2>
          </div>
