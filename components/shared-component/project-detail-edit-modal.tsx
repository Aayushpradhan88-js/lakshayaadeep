...
type ProjectDetailEditModalProps = {
...
export function ProjectDetailEditModal({ project, onClose, onSaved, showToast }: ProjectDetailEditModalProps) {
  const [form, setForm] = useState({
    title: project.title || project.project_title || "",
    description: project.description || "",
    category: project.category || "",
    organizer: project.project_organizer || "",
    status: project.status || "Draft",
    province: project.project_location?.province || "",
    district: project.project_location?.district || "",
    tole: project.project_location?.municipality || "",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (coverFile) {
        const folder = `projects/${project.id}/${Date.now()}`
        const path = `${folder}/${sanitizeFileName(coverFile.name)}`
        const { error: uploadErr } = await supabase.storage.from("project_gallery").upload(path, coverFile, {
          cacheControl: "3600",
          upsert: false,
      ...
      if (locationId) {
        ...
        await supabase.from("project").update({ location_id: loc.id }).eq("id", project.id)
      }

      if (removedGalleryIds.length > 0) {
        const { error: delErr } = await supabase.from("project_gallery").delete().in("id", removedGalleryIds)
        if (delErr) throw new Error(`Gallery delete failed: ${delErr.message}`)
      }

      const newGalleryUrls: string[] = []
      if (newGalleryFiles.length > 0) {
        const folder = `projects/${project.id}/gallery/${Date.now()}`
      ...
      const { data: updated, error: updateErr } = await supabase
        ...
        .update({
          ...
          project_organizer: form.organizer.trim(),
          status: form.status,
      ...
      onSaved(updated as ProjectRecord)
      onClose()
    } catch (error) {
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2">
        <Field label="Organizer">
          <Input value={form.organizer} onChange={(e) => set("organizer", e.target.value)} className="h-9 text-sm" />
        </Field>
      ...
      <Field label="Status">
        <Select value={form.status} onValueChange={(v) => set("status", v)}>
          ...
          <SelectContent>
            {["Draft", "Ongoing", "Upcoming", "Completed", "Cancelled"].map((s) => (
