...
export function BlogCreateForm({ onSuccess, onCancel, showToast }: BlogCreateFormProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { data, error: insertError } = await supabase.from("blogs").insert(values).select().single()
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-2 pt-2">
        ...
        <button type="submit" disabled={submitting} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-60">
          {submitting ? <FastLoading size="sm" variant="light" /> : <><FaSave className="h-4 w-4" /> Create Blog</>}
        </button>
      </div>
    </form>
  )
