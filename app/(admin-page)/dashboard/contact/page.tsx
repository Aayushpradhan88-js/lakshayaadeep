...
export default function ContactPage() {
  ...
  const updateContactStatus = async (contactId: string, status: 'pending' | 'in-progress' | 'resolved', silent = false) => {
    try {
      const supabase = getSupabaseClient();
  ...
  const deleteContact = (contactId: string) => {
    askConfirm('Are you sure you want to delete this contact message?', async () => {
      try {
        const supabase = getSupabaseClient();
        const { error } = await supabase
          .from('contacts')
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      await updateContactStatus(selectedContact.id, 'in-progress', true);
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div className="mb-4 sm:mb-0">
            ...
            <p className="text-black">Manage and respond to volunteer application submissions from your visitors</p>
          </div>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-lg">
                  <label className="text-xs font-semibold text-black uppercase tracking-wider">Name</label>
                  <p className="text-slate-900 font-medium">{selectedContact.name ?? '—'}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-lg">
                  <label className="text-xs font-semibold text-black uppercase tracking-wider">Email</label>
                  <p className="text-slate-900 font-medium">{selectedContact.email ?? '—'}</p>
                </div>
                {selectedContact.phone && (
                  <div className="bg-slate-50 p-4 rounded-lg">
                    <label className="text-xs font-semibold text-black uppercase tracking-wider">Phone</label>
              ...
              <div className="bg-slate-50 p-4 rounded-lg">
                <label className="text-xs font-semibold text-black uppercase tracking-wider">Subject</label>
                <p className="text-slate-900 font-medium mt-1">{selectedContact.subject ?? 'Contact'}</p>
              </div>

              {selectedContact.source ? (
                <div className="bg-slate-50 p-4 rounded-lg">
                  <label className="text-xs font-semibold text-black uppercase tracking-wider">Source</label>
              ...
              {selectedContact.social_primary ? (
                <div className="bg-slate-50 p-4 rounded-lg">
                  <label className="text-xs font-semibold text-black uppercase tracking-wider">Primary social</label>
              ...
              {selectedContact.social_additional ? (
                <div className="bg-slate-50 p-4 rounded-lg">
                  <label className="text-xs font-semibold text-black uppercase tracking-wider">Additional social links</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-lg">
                  <label className="text-xs font-semibold text-black uppercase tracking-wider">Received</label>
      ...
      {showReplyModal && selectedContact && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              <div className="bg-slate-50 p-4 rounded-lg">
                <label className="text-xs font-semibold text-black uppercase tracking-wider">To</label>
