...
export default function CreateeventPage() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      // Upload cover image to event_cover_image_url bucket (first image)
      if (images.length > 0) {
        ...
        const { error: coverErr } = await supabase.storage.from('event_cover_image_url').upload(coverPath, coverFile, {
          cacheControl: '3600', upsert: false, contentType: coverFile.type || 'image/jpeg',
        })
      ...
      // Upload remaining images to event_gallery bucket
      ...
      // Insert gallery images (including cover image for reference)
      if (event?.id) {
        const galleryEntries = []
        if (coverImageUrl) {
          galleryEntries.push({ event_id: event.id, image_url: coverImageUrl })
        }
  ...
  return (
    <div className="max-w-xl mx-auto px-5 py-8">
      <h2 className="text-xl font-semibold text-center mb-7">Create event</h2>

      {/* Title */}
      <Field label="Title" error={errors.title}>
        <Input
          placeholder="Write your event agenda"
          value={form.title}
      ...
      <Field label="Description">
        <Textarea
          rows={4}
          placeholder="Describe the event goals and impact..."
